...
class TelegramBot:
    ...
    def _post(self, method, data):
        try:
            resp = requests.post(f"{self.base_url}/{method}", json=data)
            return resp.json()
    ...
    def answer_callback_query(self, callback_query_id, text=None, show_alert=False):
        ...
        return self._post("answerCallbackQuery", data)

# Bot instance will be initialized in on_fetch
bot = None
...
def start_game(p1, p2):
    ...
    for pid in [p1, p2]:
        ...
        bot.send_message(pid, msg, reply_markup=get_symbols_keyboard([]))
...
def finish_round(game):
    ...
    if game.is_game_over():
        ...
        for pid in [p1, p2]:
            ...
            bot.send_message(pid, msg, reply_markup=get_main_menu_keyboard())
        ...
        for pid in [p1, p2]:
            if pid == AI_USER_ID: continue
            bot.send_message(pid, f"⚔️ <b>Round {game.current_round} begins!</b>", reply_markup=get_symbols_keyboard([]))
...
async def process_update(update):
    if "message" in update:
        ...
        if text.startswith("/start"):
            ...
            bot.send_message(chat_id, welcome, reply_markup=get_main_menu_keyboard())
        ...
        if data == "menu_join":
            if user_id in MATCHMAKING_QUEUE:
                bot.answer_callback_query(cb["id"], "Still searching...")
            else:
                MATCHMAKING_QUEUE.append(user_id)
                bot.edit_message_text(chat_id, message_id, "🛰️ <b>Searching for an opponent...</b>", reply_markup={"inline_keyboard": [[{"text": "❌ Cancel", "callback_data": "leave_queue"}]]})
            ...
            bot.edit_message_text(chat_id, message_id, "🛡️ <b>NEXUS-7 PvP</b> 🛡️\n\nMain Menu", reply_markup=get_main_menu_keyboard())

        elif data == "leave_queue":
            if user_id in MATCHMAKING_QUEUE:
                MATCHMAKING_QUEUE.remove(user_id)
                bot.edit_message_text(chat_id, message_id, "Left the queue.", reply_markup=get_main_menu_keyboard())
            ...
            if len(combo) < 3 and symbol_id not in combo:
                ...
                bot.edit_message_text(chat_id, message_id, cb["message"].get("text", "Round in progress"), reply_markup=get_symbols_keyboard(combo))
            else:
                bot.answer_callback_query(cb["id"], "Selection full.")
            ...
            combo = game.players[user_id]["combo"]
            if len(combo) != 3:
                bot.answer_callback_query(cb["id"], "Select 3 symbols!", show_alert=True)
                return
            game.set_player_combo(user_id, combo)
            bot.edit_message_text(chat_id, message_id, "⌛ <b>Waiting for opponent...</b>")
...
async def on_fetch(request, env, ctx):
    ...
    return Response.new("Nexus Bot (HTTP) is Active.")
